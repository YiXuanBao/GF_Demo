...
public class YXUtils
{
    ...
    public static void InvokeAction(float seconds, Action callback, int repeatCount = 0, float repeatInterval = 1, string id = "", Action onFinish = null)
    {
        if (callback == null)
        {
            LogError("InvokeAction: <Action callback> is null.");
            return;
        }
        //if (!CommonTimer.Instance) return;
        //CommonTimer.Instance.InvokeAction(seconds, callback, repeatCount, repeatInterval, id, onFinish);
    }
    ...
    public static void InvokeActionUnlimited(float seconds, Action callback, float repeatInterval = 1, string id = "")
    {
        ...
        //CommonTimer.Instance.InvokeActionUnlimited(seconds, callback, repeatInterval, id);
    }
    ...
    {
        //CommonTimer.Instance.CancelInvokeAction(id);
    }
    ...
    public static Coroutine StartProcedure(Curve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish = null, bool allowMultipleInstances = false, string id = "")
    {
        Coroutine cr = StartCoroutine(ProcedureCR(curve, startValue, endValue, timeParam, action, onFinish), allowMultipleInstances);
        if (id.Length > 0)
        {
            procedureDict.InsertOrUpdate(id, cr);
        }
        return cr;
    }
    ...
    /// <param name="onFinish">Action called at the end of the procedure.</param>
    /// <param name="id"></param>
    public static Coroutine StartProcedure(Curve curve, float time, Action<float> action, Action<float> onFinish = null, string id = "")
    {
    ...
    /// <param name="onFinish">Action called at the end of the procedure.</param>
    /// <param name="id"></param>
    public static Coroutine StartProcedureUnscaled(Curve curve, float time, Action<float> action, Action<float> onFinish = null, string id = "")
    {
    ...
    private static IEnumerator ProcedureCR(Curve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish)
    {
        ...
        for (int i = 0; i < step; i++)
        {
            variable = startValue + (YXCurveSampler.SampleCurve(curve, i / step)) * diff;
            action?.Invoke(variable);
    ...
    private static IEnumerator ProcedureCRUnscaled(Curve curve, float startValue, float endValue, float timeParam, Action<float> action, Action<float> onFinish)
    {
    ...
    public static Coroutine StartCoroutine(IEnumerator cr, bool allowMultipleInstances = false, string id = "")
    {
        if (!allowMultipleInstances)
        {
            ...
            {
               // CommonTimer.Instance.StopCoroutine(crDict[nameof(cr)]);
            }
            crDict.InsertOrUpdate(nameof(cr), cr);
        }
        if (id.Length > 0)
            crDict.InsertOrUpdate(id, cr);
        return null;//  CommonTimer.Instance.StartCoroutine(cr);
    }
    ...
    {
        //CommonTimer.Instance.StopCoroutine(cr);
    }
    /// <summary>
    /// Stops a coroutine.
    /// </summary>
    ...
    public static void StopCoroutine(string id)
    {
        if (id.Length > 0)
            if (crDict.ContainsKey(id))
                ;// CommonTimer.Instance.StopCoroutine(crDict[id]);
    }
    /// <summary>
    /// Stops a coroutine.
    /// </summary>
    ...
    {
        //if (CommonTimer.Instance) //Batch v0.15.3: fixed issue with SKTextAnimator errors when quitting game
        //    CommonTimer.Instance.StopCoroutine(cr);
    }
    #endregion
}
